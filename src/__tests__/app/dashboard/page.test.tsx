...
describe("Dashboard Page", () => {
  beforeEach(() => {
    ...
    mockFetch
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          stats: { activePlans: 0, weeklyMiles: 0, currentStreak: 0 },
        }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
  ...
  describe("when user is authenticated", () => {
    ...
    it("displays welcome message with user name", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Welcome back, John Runner!")
        ).toBeInTheDocument();
      });
      expect(
        screen.getByText(/Track your marathon training progress/)
      ).toBeInTheDocument();
    });
    ...
    it("displays quick action navigation", () => {
      ...
      expect(
        screen.getByText("🏃‍♂️ Log Workout (Coming Soon)")
      ).toBeInTheDocument();
      expect(
        screen.getByText("📊 View Analytics (Coming Soon)")
      ).toBeInTheDocument();
    });
    ...
    it("has correct links for quick actions", () => {
      ...
      expect(
        screen.getByRole("link", { name: "📅 View Training Plans" })
      ...
      const workoutLink = screen
        .getByText("🏃‍♂️ Log Workout (Coming Soon)")
        .closest("a");
      expect(workoutLink).toHaveAttribute("href", "#");

      const analyticsLink = screen
        .getByText("📊 View Analytics (Coming Soon)")
        .closest("a");
    ...
    it("displays quick stats with zero values", async () => {
      ...
      expect(
        screen.getByText("Training plans in progress")
      ).toBeInTheDocument();
