...
describe("NewPlanForm", () => {
  ...
  describe("Form Rendering", () => {
    it("renders form elements correctly", () => {
      ...
      expect(
        screen.getByText("Create Marathon Training Plan")
      ).toBeInTheDocument();
      expect(screen.getByLabelText("Marathon Date")).toBeInTheDocument();
      expect(
        screen.getByLabelText("Peak Weekly Mileage (miles)")
      ).toBeInTheDocument();
      expect(
    ...
    it("has HTML5 validation for required fields", () => {
      ...
      const marathonDateInput = screen.getByLabelText(
        "Marathon Date"
      ) as HTMLInputElement;
      const peakMileageInput = screen.getByLabelText(
    ...
    it("shows form description", () => {
      ...
      expect(
        screen.getByText(/This will create an 18-week training plan/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/ending on your marathon date/)
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Form Interactions", () => {
    ...
    it("requires both fields to be filled before submitting", async () => {
      ...
      expect(
        screen.queryByText("Creating your training plan...")
    ...
    it("shows loading state during form submission", async () => {
      ...
      expect(
        screen.getByText("Creating your training plan...")
      ).toBeInTheDocument();
  ...
  describe("Error Handling", () => {
    it("shows error message when submission fails", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Failed to create plan. Please try again.")
        ).toBeInTheDocument();
      });
    });
    ...
    it("clears error message on new submission", async () => {
      ...
      expect(
        screen.queryByText("Failed to create plan. Please try again.")
      ).not.toBeInTheDocument();
      expect(
        screen.getByText("Creating your training plan...")
      ).toBeInTheDocument();
  ...
  describe("Authentication", () => {
    it("shows error when user is not authenticated", () => {
      ...
      expect(
        screen.getByText("You must be logged in to create a training plan.")
      ).toBeInTheDocument();
