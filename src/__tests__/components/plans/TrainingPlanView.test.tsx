...
describe("TrainingPlanView", () => {
  ...
  it("displays weeks and training days correctly", () => {
    ...
    expect(screen.getByText("Target: 25 miles")).toBeInTheDocument();
    expect(screen.getByText("Target: 30 miles")).toBeInTheDocument();
  });
  ...
  describe("Plan Duplication", () => {
    ...
    it("shows duplicate dialog when duplicate button is clicked", () => {
      ...
      expect(
        screen.getByText("Duplicate Training Plan")
      ).toBeInTheDocument();
...
describe("TrainingPlanView - Training Day Editing", () => {
  ...
  it("should populate edit form with current day values", () => {
    ...
    expect(milesInput.value).toBe("8");
    expect(descriptionInput.value).toBe("Easy Run");
  });
  ...
  it("should update training day when save is clicked", async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith("/api/training-days/day-1-1", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  it("should allow editing only one day at a time", () => {
    ...
    // Try to click second edit button - it should not be visible
    expect(screen.getAllByText("Edit Day").length).toBe(
